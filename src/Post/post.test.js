...
test("Delete Button Getting Fired with Correct Params", () => {
  const fakeData = {
    title: "",
    url: "",
    imageUrl: "",
  ...
  const { getByText } = render(<Post {...fakeData} />);
  const deleteButton = getByText("Delete");
  Simulate.click(deleteButton);
  expect(fakeData.onDelete).toHaveBeenCalledTimes(1);
  expect(fakeData.onDelete).toHaveBeenCalledWith(fakeData.id);
});

test("Image Url Undefined", () => {
  const fakeData = {
    title: "",
    url: "",
    imageUrl: undefined,
  ...
  const { container } = render(<Post {...fakeData} />);
  const imgNode = container.querySelector("img");
  expect(imgNode.src).toBe(defaultImg);
});
